...
const numCheckBtn = document.getElementById('num-check-btn')
const numForgotBtn = document.getElementById('num-forgot-btn')
const numRichtig = document.getElementById('num-richtig')
const numFalsch = document.getElementById('num-falsch')
...
numBtn.addEventListener('click', function () {
    ...
    count ++
    Weiter(count, numBtn)
    const randomNum = getRandomNumber(NumsArray.length)
    numText.textContent = NumsArray[randomNum][0]
    numValue = NumsArray[randomNum][1]
    numRichtig.classList.remove('active')
    numFalsch.classList.remove('active')
    numInput.value = ''
})
numCheckBtn.addEventListener('click', function () {
    ...
    numValueStr = numValueStr.trim().toLowerCase()
...
const sentencesArray = [
    ...
    ['Я новичек в этом доме.', 'Ich bin neu hier im Haus.']
]
...
sentCheckBtn.addEventListener('click', function () {
    ...
    if (sentValueStr[sentValueStr.length - 1] === ' ') {
        sentValueStr = sentValueStr.slice(0, -1)
    }
    ...
    sentValueStr = sentValueStr.replace(/ \./g, '.').replace(/\s+/g, ' ')
